...
export function notifyDragStarted(frameId?: number):void {
    ...
    const htmlToImageOptions: Options = {backgroundColor: "transparent", pixelRatio: companionImgScalingRatio};
    let cropOptions = {} as {x: number, y: number, width: number, height: number};
    ...
    if(frameId){
        ...
        if(useStore().frameObjects[frameId].frameType.type == AllFrameTypesIdentifier.comment){
            ...
            htmlToImageOptions.backgroundColor = commentBackgroundColor;
        }
    }
    else{
        ...
        cropOptions = {...getHTML2CanvasFramesSelectionCropOptions(useStore().frameObjects[useStore().selectedFrames[0]].parentId)};
        renderingCanvas.width = cropOptions.width * companionImgScalingRatio;
        renderingCanvas.height = cropOptions.height * companionImgScalingRatio;
    ...
    if(draggingEl && draggingEl.id != "fdsfsfsd"){ 
        try{
            htmlToImage.toPng(draggingEl, htmlToImageOptions).then((compImgDataUrl: string) => {
                console.log(compImgDataUrl);
                // We set the image generated by html-to-image in our renderning canvas.
                // IF THERE WAS A SELECTION, we need to crop the image correctly based on the cropping options.
                const renderingCanvasContext = renderingCanvas.getContext("2d");
                const compImg = new Image();
                // When the image is loaded, draw it on the canvas
                compImg.onload = () => {
                    renderingCanvasContext?.drawImage(compImg, (frameId) ? 0 : cropOptions.x, (frameId) ? 0 : cropOptions.y, renderingCanvas.width, renderingCanvas.height, 0,0, renderingCanvas.width, renderingCanvas.height);
                };
                // Set the image source to the base64 URL
                compImg.src = compImgDataUrl;                                 
            }, () => {});    
        }
        catch (error){
            // There seem to be a few Security errors with html-to-image related to CORS. We just report them in the console.
            console.log("Error with using html-to-image.");
        }
    }
}
