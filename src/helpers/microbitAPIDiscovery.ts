...
const compileTextualAPI = function(apiCodedItems: APICodedItem[], level?: number, immediateParentName?: string): APIItemTextualDescription[] {
    ...
    apiCodedItems.forEach((apiItem) => {
        ...
        const shortDoc = (i18n.global.te("apidiscovery.microbitAPI."+apiItem.name+"_doc","en")) ? i18n.global.t("apidiscovery.microbitAPI."+apiItem.name+"_doc") as string : "";
        const extraDoc = (i18n.global.te("apidiscovery.microbitAPI."+apiItem.name+"_extradoc","en")) ? i18n.global.t("apidiscovery.microbitAPI."+apiItem.name+"_extradoc") as string : "";
        ...
        apiDocumentedItems.push({name: apiItem.name,
            label: i18n.global.t("apidiscovery.microbitAPI."+apiItem.name+"_label") as string,
            doc: shortDoc,
            extradoc: extraDoc,
            version: version,
            level: level??1,
